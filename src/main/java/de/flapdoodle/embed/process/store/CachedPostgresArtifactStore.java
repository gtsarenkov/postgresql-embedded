...
public class CachedPostgresArtifactStore extends PostgresArtifactStore {
    ...
    public CachedPostgresArtifactStore(DownloadConfig downloadConfig, Directory eDir,
                                       TempNaming executableNaming, Downloader downloader) {
    ...
    public Optional<ExtractedFileSet> extractFileSet(Distribution distribution) {
        try {
            ...
            if (exists(path)) {
                final Builder extracted = ImmutableExtractedFileSet.builder(dir).baseDirIsGenerated(false);
                ...
                return Optional.of(extracted.build());
            } else {
